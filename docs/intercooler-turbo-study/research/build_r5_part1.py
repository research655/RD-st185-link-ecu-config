...
OLD = open("intercooler-report.html", encoding="utf-8").read()
CSS = re.search(r"<style>.*?</style>", OLD, re.S).group(0)
R4 = json.loads(re.search(r"var R4=(\{.*?\});\n", OLD, re.S).group(1))
...
number and it falls straight out of one measurement. See &sect;07 for the width table.</p>
</div>
</div>
...
<td><span class="pill p-ok">Confirmed &mdash; validated in &sect;12</span></td></tr>
...
is pressure loss and volume, and both are computed directly in &sect;06.</p>
    ...
    for a in R5["aperture"]:
        note = ("Aperture is the binding constraint. Go to 114 mm thick to buy some of it back."
                ...
                ("Wider than this stops paying &mdash; the radiator core is only 699 mm."
                 if a["w"] == 685 else "Standard recommendation."))
...
charge. That crossover is at 114 mm, so 102 mm is the last comfortable step. See &sect;09.</p>
...
A/C condenser behind it. See &sect;08.</li>
</ul>
</div>
</div>
...
kept in &sect;13 purely as a record of what was considered and rejected.</div>
...
def turbo_survey():
    ...
    for t in R4.get("turbos", []) or []:
        pass
    old = json.loads(re.search(r"var R3=(\{.*?\});\n", OLD, re.S).group(1))["turbos"]
    for t in old:
...
One sentence in &sect;06 now covers it.</td></tr>
...
not a constraint. &sect;07 specifies the core on merit; &sect;14 lists places that will build it.</td></tr>
...
def pkg_rows():
    ...
    for p in PKG:
        ...
        out.append("<tr%s><td><b>%s</b><br><span class=\"note\">%s</span></td>"
                   ...
                   "<td class=\"num\">%.2f</td><td>%d &times; %d &times; %d</td>"
                   ...
                   % (' style="background:#12251f"' if best else "", p["label"], p["note"],
                      p["hot"], p["cold"], max(p["hot"], p["cold"]) if p["cold"] != p["hot"] else p["cold"],
...
<p class="lede">This turned out to matter more than the charge pipe does. Half a pound of inlet
...
the entries that land on the aperture table in &sect;07, so you can pick the part number the moment
...
<td>Narrow aperture, buying depth back. This is the "under 460 mm" row in &sect;07.</td>
<td class="num">&mdash; <span class="pill p-warn">unverified</span></td></tr>
...
<tr><td>Custom build to the &sect;07 spec</td><td>your aperture &times; 305 &times; 102</td>
...
It is the only path that gives you all of &sect;07.</p>
</div>
<div class="card">
...
up the exact core width, you take 114 mm depth instead of 102, and you take two port transitions.
The model says that costs about 1 whp. It is a defensible trade.</p>
</div>
</div>

<div class="callout c-info"><b>The difference between the two paths is about $300 and a few weeks,
